...
import { calculateObjectCount, logDensityVerification, logCameraPosition, createMaterial, createFrameRateLimitedAnimation } from './utils.js';
...
async function initializeScene() {
    try {
        ...
        loadedModels.forEach((model, modelIndex) => {
            ...
            // Create a base material using the configured material type
        ...
        const animateFunction = () => {
            ...
            if (Math.floor(elapsedTime) % 5 === 0 && Math.floor(elapsedTime) !== animateFunction.lastLoggedTime) {
                logCameraPosition(camera);
                animateFunction.lastLoggedTime = Math.floor(elapsedTime);
            }
            ...
            // Camera panning animation (if enabled)
            if (CAMERA_CONFIG.ENABLE_AUTO_PAN) {
                const panAngle = elapsedTime * CAMERA_CONFIG.PAN_SPEED;
                camera.position.x = Math.cos(panAngle) * CAMERA_CONFIG.PAN_RADIUS;
                camera.position.z = Math.sin(panAngle) * CAMERA_CONFIG.PAN_RADIUS;
                camera.position.y = CAMERA_CONFIG.PAN_HEIGHT;
                
                // Make camera look at the center of the falling objects
                camera.lookAt(0, OBJECT_CONFIG.Y_OFFSET, 0);
            }

            // Update the controls (if enabled and auto-pan is disabled)
            if (controls && !CAMERA_CONFIG.ENABLE_AUTO_PAN) {
                controls.update();
            }
        ...
        // Start the animation with optional frame rate limiting
        if (ANIMATION_CONFIG.ENABLE_FPS_LIMIT) {
            createFrameRateLimitedAnimation(animateFunction, ANIMATION_CONFIG.TARGET_FPS);
        } else {
            const animate = () => {
                animateFunction();
